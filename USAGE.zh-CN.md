...
| 服务端自动续期 | 由 Arova 服务端维持，Chrome 关闭后仍可工作 | 在来源卡片填写材料（Fomo 可主动确认后读取填充），阅读风险并明确确认；入口须由服务端开放 |
...
1. 在同一 Chrome 配置中打开 `https://fomo.family`，登录需要接管的 Fomo 账号。
2. 在 Arova 管理页 → 监控授权 → Fomo → 服务端续期，点击“从已登录的 Fomo 页面读取”。
3. 选择官网标签页，阅读并勾选本次读取同意，点击“确认读取并填充”；Chrome 提示可选权限时按需允许。
4. 两项必填齐全时，三个输入框整体替换为本次会话；可选 PAT 不存在时留空。此时不上传，也不会启动续期。
5. 核对账号和输入，按原提示关闭共享会话的官网页面，再单独勾选托管同意并提交。服务端仍会验证是否为已连接的同一账号。

缺少或无法识别必填字段时不会填充任何字段，原输入保留，可按按钮下方的说明手动填写。官网存储方式可能变化，已登录也不保证能读取这些字段。页面刷新、跳转、账号改变或拒绝权限会取消读取。读取不会自动关页、刷新 Token 或读取钱包密钥。
